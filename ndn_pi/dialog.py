...
class Dialog(object):
    ...
    def prompt(self, msg, default='', preExtra=(), password=False):
        control = 'passwordbox' if password else 'inputbox'
        return self.run(control, msg, postExtra=[default], preExtra=preExtra)
    ...
    def mainMenu(self, msg='', items=(), preExtras=(), prefix = ' ', postExtras=(),
        ...
        allPreExtras.extend(['--no-cancel', '--ok-label', okLabel])
    ...
    def form(self, msg='', formFieldInfo=[], preExtras=(), postExtras=(), 
        ...
        # need to remove the trailing newline though
        response = Response(response.returncode, 
            [v.strip() for v in response.value[:-1].split('\n')])
        return response
